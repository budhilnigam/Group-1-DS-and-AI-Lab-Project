"""Retrieve top-k chunks from the FAISS index with optional deduplication.

Supports two modes:
  1. retrieve(query, top_k) — basic single-query FAISS retrieval.
...
def load_index() -> tuple[faiss.Index, list[dict]]:
    if not INDEX_PATH.exists() or not META_PATH.exists():
        raise FileNotFoundError(
            f"FAISS index not found. Run build_faiss_index.py first.\n"
...
def retrieve(
    ...
    for score, idx in zip(scores[0], idxs[0]):
        ...
        # reconstruct the stored vector for dedup comparison
        if deduplicate:
            vec = np.zeros(index.d, dtype=np.float32)
            index.reconstruct(int(idx), vec)
            candidate_embs.append(vec)

    if deduplicate and candidates:
...
def retrieve_with_context(
    ...
    Three-stage pipeline:
      ...
      3. **Rerank**: boost FAISS scores for chunks whose category matches
